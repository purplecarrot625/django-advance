...
def recipe_image_file_path(instance, filename):
    """ Generate file path for new recipe image """
    ext = os.path.splitext(filename)[1] # get the extension of the file, eg. .jpg
    filename = f'{uuid.uuid4()}{ext}'

    return os.path.join('uploads', 'recipe', filename) # Create the path for different OS
...
class Recipe(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,  # settings.py file
        on_delete=models.CASCADE,
    )  # set the user that the recipe belongs to
